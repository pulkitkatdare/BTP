...
% fopen(sp);
x=[1 1 1 1 1 1 1 1 1 ]'*20;
y=[1 1 1 1 1 1 1 1 1 ]'*0;
P_init=[x y];
h_init=pi/2*ones(1,9);
...
% [pos_cor_x pos_cor_y]
                        ...
                        if(ID<=10) 
                        p_data{ID}=[p_data{ID}; [Position Heading ID  ] ];
...
%                 h_st=[911 912 913 914 915 916 917 918 919 920];%T_cpu
...
%         pause(1);
%         pkt_1= xbee_send( h,x,y);
        if (zigbeeFlag)
            xbee_send( sp, h, x, y, T_cpu);
